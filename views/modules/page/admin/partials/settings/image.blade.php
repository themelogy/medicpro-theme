    ...
    <fieldset style="padding: 0 20px;">
        ...
        <div class="row">
            ...
            <div class="col-md-12 form-inline">
                ...
                <div class="form-group{{ $errors->has("settings.image_class") ? ' has-error' : '' }}">
                    {!! Form::input('text', 'settings[image_class]', !isset($page->settings->image_class) ? null : $page->settings->image_class, ['class'=>'form-control', 'placeholder'=>'Resim Class']) !!}
                    {!! $errors->first("settings.image_class", '<span class="help-block">:message</span>') !!}
                </div>
            </div><br/>
            <hr/>
            <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group{{ $errors->has("settings.list_image_width") ? ' has-error' : '' }}">
                            {!! Form::input('text', 'settings[list_image_width]', !isset($page->settings->list_image_width) ? 400 : $page->settings->list_image_width, ['class'=>'form-control', 'placeholder'=>'Liste Resim Genişliği']) !!}
                            {!! $errors->first("settings.list_image_width", '<span class="help-block">:message</span>') !!}
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group{{ $errors->has("settings.list_image_height") ? ' has-error' : '' }}">
                            {!! Form::input('text', 'settings[list_image_height]', !isset($page->settings->list_image_height) ? null : $page->settings->list_image_height, ['class'=>'form-control', 'placeholder'=>'Liste Resim Yüksekliği']) !!}
                            {!! $errors->first("settings.list_image_height", '<span class="help-block">:message</span>') !!}
                        </div>
                    </div>
                    <div class="col-md-3">
                        {!! Form::select('settings[list_image_mode]', [null=>'Varsayılan', 'fit'=>'Kırp', 'resize'=>'Ölçeklendir'],!isset($page->settings->list_image_mode) ? '' : $page->settings->list_image_mode, ['class'=>'form-control']) !!}
                        {!! $errors->first("settings.list_image_mode", '<span class="help-block">:message</span>') !!}
                    </div>
                </div>
            </div>
        </div>
